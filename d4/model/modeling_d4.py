...
class LLaDATalkModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        super().__init__()
        self.config = config
        self.__cache = BufferCache()
    ...
    def forward(
        ...
        # concatenate the embedded input with the representation from thought model
        # x = torch.cat([x, input_rps], dim=-1)
        # print(x.shape)
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    if block_idx == 0:
                        x, cache = self._activation_checkpoint_fn(
                            block, x, hidden_states=input_rps, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position
                        )
                    else:
                        # shape: (batch_size, seq_len, d_model)
                        x, cache = self._activation_checkpoint_fn(
                    ...
                    if block_idx == 0:
                        x, cache = block(x, hidden_states=input_rps, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
                    else:
                        # shape: (batch_size, seq_len, d_model)
                        x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position)
                if attn_key_values is not None:
